...
export default function Calendar() {
  ...
  const eventDays = [10, 15, 22] // Days with events
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-lg p-8">
          ...
          <div className="grid grid-cols-7 gap-1 mb-8">
            {/* Day Headers */}
            {dayNames.map((day) => (
              <div key={day} className="bg-gray-100 p-3 text-center font-semibold text-blue-600 text-sm">
                {day}
              </div>
            ))}
                ...
                className={`p-3 text-center text-sm min-h-[48px] flex items-center justify-center cursor-pointer transition-colors ${
                  dayObj.isEmpty 
                    ...
                    : dayObj.isToday
                      ...
                      : dayObj.hasEvent
                        ? 'bg-green-100 text-blue-600 font-bold hover:bg-green-200 rounded-lg'
                        : 'hover:bg-gray-100 rounded-lg'
                }`}
                title={dayObj.hasEvent ? 'Event scheduled' : ''}
              >
                {dayObj.day}
              </div>
            ))}
